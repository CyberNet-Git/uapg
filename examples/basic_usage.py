...
async def run_opcua_server():
    ...
    server.set_server_name("Sin OPC UA Server with History")
    ...
    # Set up history storage with TimescaleDB and connection pool
    ...
    await vart.set_modelling_rule(True)
 #   await server.nodes.objects.add_object(idx, "MyDeviceInstance", dev)
    ...
    async with server:
        ...
        print(f"History storage initialized with connection pool: {history._min_size}-{history._max_size} connections")
